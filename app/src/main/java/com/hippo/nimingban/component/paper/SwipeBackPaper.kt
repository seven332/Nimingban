...
class SwipeBackPaper(
    private val logic: SwipeBackLogic
) : GroupPaper<SwipeBackPaper>(logic), SwipeBackUi {

  companion object {
    const val CONTAINER_ID = R.id.swipe_back_container
  }

  private lateinit var swipeBack: SwipeBackLayout

  override fun onCreate(inflater: LayoutInflater, container: ViewGroup) {
    super.onCreate(inflater, container)

    view = inflater.inflate(R.layout.paper_swipe_back, container, false)
    swipeBack = view.find(R.id.swipe_back)
