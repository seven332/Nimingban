...
import com.hippo.nimingban.util.LayoutKt;
import com.hippo.nimingban.util.MathKt;
...
public class Slider extends View {
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Slider, R.attr.sliderStyle, 0);
    ...
    mBgPaint.setColor(a.getColor(R.styleable.Slider_bgColor, Color.BLACK));

    setReverse(a.getBoolean(R.styleable.Slider_reversed, false));

    a.recycle();

    mProgressAnimation = new ValueAnimator();
    mProgressAnimation.setInterpolator(Animators.FAST_SLOW);
    mProgressAnimation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
      @Override
      public void onAnimationUpdate(@NonNull ValueAnimator animation) {
        float value = (Float) animation.getAnimatedValue();
        mDrawPercent = value;
        mDrawProgress = Math.round(MathKt.lerp(value, mStart, mEnd));
  ...
  private void updateBubbleSize() {
    ...
    mBubbleWidth = (int) Math.max(mBubbleMinWidth,
        Integer.toString(mEnd).length() * mCharWidth + LayoutKt.dp2pix(8, mContext));
    mBubbleHeight = (int) Math.max(mBubbleMinHeight,
        mCharHeight + LayoutKt.dp2pix(8, mContext));
  ...
  private void updatePopup() {
    ...
    mPopupY = (int) (mTemp[1] - popupHeight + paddingTop +
        ...
        mRadius - LayoutKt.dp2pix(2, mContext));
  ...
  private void startProgressAnimation(float percent) {
    ...
    mProgressAnimation
        .setDuration(Math.min(500, (long) (50 * getWidth() * Math.abs(currentValue - percent))));
  ...
  private void startShowBubbleAnimation() {
    ...
    mBubbleScaleAnimation.setInterpolator(Animators.FAST_SLOW);
  ...
  private void startHideBubbleAnimation() {
    ...
    mBubbleScaleAnimation.setInterpolator(Animators.SLOW_FAST);
  ...
  protected void onDraw(@NonNull Canvas canvas) {
    ...
    if (width < LayoutKt.dp2pix(24, mContext)) {
      ...
      float currentX =
          paddingLeft + radius + (width - radius - radius - paddingLeft - paddingRight) *
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_CANCEL:
        ...
        int progress = Math.round(
            MathKt.lerp(
                MathKt.clamp(
                    (mReverse ? (getWidth() - paddingLeft - radius - x)
                        : (x - radius - paddingLeft)) / (getWidth() - radius - radius - paddingLeft - paddingRight),
                    0.0f, 1.0f),
                (float) mStart,
                (float) mEnd));

        float percent = MathKt.delerp(progress, mStart, mEnd);
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
          removeCallbacks(mCheckForShowBubble);
          setShowBubble(false);
        } else if (action == MotionEvent.ACTION_DOWN) {
          if (mCheckForShowBubble == null) {
            mCheckForShowBubble = new CheckForShowBubble();
          }
          postDelayed(mCheckForShowBubble, ViewConfiguration.getTapTimeout());
        }
  ...
  private static class BubbleView extends AppCompatImageView {
    ...
    public BubbleView(Context context, Paint paint) {
      super(context);
      setImageResource(R.drawable.slider_bubble);
  ...
  interface OnSetProgressListener {
